...
export default function EnrollmentGate({courseId, courseTitle, children, catalogUrl}: EnrollmentGateProps) {
  ...
  useEffect(() => {
    ...
    async function verifyAccess() {
      try {
        ...
        const response = await fetch(`${fields.accessApi}?courseId=${encodeURIComponent(courseId)}`, {method: 'GET', credentials: 'include', headers, signal: controller.signal});
        if (response.ok) {
          const result = (await response.json()) as {allowed?: boolean; reason?: string};
          if (result.allowed === true) { setState('allowed'); return; }
          setState('locked');
          setDetail(result.reason || 'Your account is signed in, but an active enrolment was not found for this course.');
          return;
        }
        if (response.status === 401) { setState('locked'); setDetail('Sign in with the learner account used during registration or enrolment.'); return; }
        if (response.status === 403 || response.status === 404) { setState('locked'); setDetail('An active enrolment was not found for this learner and course.'); return; }
  ...
  return <section className="accessGate" role="status" aria-live="polite">
